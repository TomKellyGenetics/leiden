...
This package allows calling the Leiden algorialgorithmthm for clustering on an igraph object from R. See the Python and Java implementations for more details: 
...
conda_install(env = "r-reticulate", c("leidenalg", "python-igraph"), forge = TRUE, pip = TRUE)
```
...
```{bash, eval = FALSE}
#pip3 uninstall -y igraph
pip3 install -U -q leidenalg python-igraph
```
